...
- 固定接口配置：所有模型渠道的 Base URL 固定为 `https://img.tokenshen.top` 且输入框禁用，需验证历史配置和新增渠道也无法覆盖。
- 首页落地页：首屏改为“无限画布”品牌大字、动态创作输入框与真实画布截图，输入内容后可带入生图工作台；需验证桌面端与移动端布局、深浅主题、打字机效果、入口跳转和图片预览正常。
