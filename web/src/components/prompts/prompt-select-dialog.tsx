...
import { ALL_PROMPTS_OPTION, fetchPrompt, type PromptListItem } from "@/services/api/prompts";
            ...
            <div data-canvas-no-zoom onWheelCapture={(event) => event.stopPropagation()}>
                ...
                <div className="thin-scrollbar mt-6 max-h-[520px] overflow-y-auto pr-2" data-canvas-no-zoom onScroll={handleListScroll} onWheelCapture={(event) => event.stopPropagation()}>
                    ...
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                        {items.map((item) => (
                            <PromptCard key={item.id} item={item} loading={loadingPromptId === item.id} onOpen={() => selectPrompt(item)} onCopy={() => selectPrompt(item)} actionLabel="使用此提示词" actionIcon={<Check className="size-3.5" />} actionType="primary" />
                        ))}
                    </div>
